...
| D1.4.1 | Comply with WCAG 2.2 AA or higher? | **Yes** — the platform is designed to support and tested against **WCAG 2.2 AA**. Automated accessibility testing using `axe-core` is integrated into the test suite and runs against key page templates, with local validation available via `s/test --a11y-only`. See [Accessibility](/docs/accessibility/). |
| D1.4.2 | Timescale to obtain WCAG 2.2 AA | Current automated test coverage targets WCAG 2.2 AA. Continue periodic manual assistive-technology review and update the accessibility statement as deployments are assessed. |
