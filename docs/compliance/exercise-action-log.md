...
| **MFA Redundancy:** Scenario showed that if the CTO's primary phone is lost, Northflank access is delayed. | Provision and test a secondary hardware security key (YubiKey) for Northflank. | CTO | 15/12/25 | **Complete** | 10/12/25 | Successful test login recorded in audit log. |
