...
**Status:** Resolved. `HealthcareOIDCCallbackView` (`checktick_app/core/oidc_views.py`) no longer mutates the process-global `django.conf.settings` object to switch between Google and Azure providers. The previous try/finally block that stored originals, overwrote `settings.OIDC_RP_CLIENT_ID` / `OIDC_RP_CLIENT_SECRET` / `OIDC_OP_TOKEN_ENDPOINT` / `OIDC_OP_USER_ENDPOINT` / `OIDC_OP_JWKS_ENDPOINT` / `OIDC_RP_SCOPES` with Azure values, called `super().get()`, and restored in `finally` has been removed entirely. Instead, the callback view now overrides `get_settings(attr, *args)` (mirroring the existing pattern already used by `HealthcareOIDCAuthView`) to resolve provider-specific config per-request from the `OIDC_PROVIDERS` dict in `settings.py`, keyed by the `oidc_provider` session value written by the auth view. This eliminates the race condition: two concurrent callbacks (one Google, one Azure) can no longer see a mix of each other's provider config. Regression tests in `checktick_app/core/tests/test_oidc_auth.py` assert (a) the Azure callback path does not mutate any of the six global OIDC settings attributes, (b) `get_settings("OIDC_RP_CLIENT_ID")` returns the Azure value when the session provider is Azure, and (c) it returns the Google value when the session provider is Google.
