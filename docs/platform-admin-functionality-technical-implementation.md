...
**Encryption at rest:** Survey responses are encrypted at submission time using a per-survey X25519 keypair (hybrid encryption: random per-response 32-byte DEK, AES-256-GCM payload, DEK wrapped via ephemeral X25519 ECDH + HKDF-SHA256). Only the survey's public key is stored server-side in plaintext form; the private key is stored wrapped and is only unwrapped into the owner's session via owner unlock (password / recovery phrase / OIDC auto-unlock / organisation key). The server can encrypt responses but cryptographically cannot decrypt them, so platform admins (and anyone with database access) cannot read response content — including demographics, which are encrypted inside the same response payload — without recovering the survey private key. Recovery flows (see [Recovery Dashboard](/docs/recovery-dashboard/)) use the same Vault escrow mechanisms and audit trails to recover the survey private key; recovering it grants the ability to decrypt all responses encrypted with the survey's public key, and is restricted to organisation owners and platform admins via the recovery dashboard's dual-authorization workflow.
