...
> **Status: Design document.** This describes a planned feature. The
> `linear` layout is the current behaviour; the `section_menu` layout is
> proposed. No models, routes, or templates exist yet for `section_menu`.
>
> Related: [Survey Progress Tracking](survey-progress-tracking.md) — the
> save-and-resume work that should land first, because a long menu-driven
> survey without resume is a poor experience.
...
class SectionMenu(models.Model):
    ...
    order_mode = models.CharField(
        choices=["authored", "participant"], default="authored",
    )
...
1. The chosen group IDs (plus the mandatory ones) are stored on the
   progress record as `selected_group_ids` (see
   [Survey Progress Tracking](survey-progress-tracking.md)).
...
| Save progress + selection | A participant who picks 3 sections, leaves, and returns must not have to re-pick. | `selected_group_ids` lives on the progress record, not in the URL. See [Survey Progress Tracking](survey-progress-tracking.md). |
...
This feature depends on the save-and-resume work landing first, because a
menu-driven survey without resume is a poor experience for long surveys.

1. **Save-and-resume** — see
   [Survey Progress Tracking](survey-progress-tracking.md). Ships value on
   its own and is a prerequisite. Includes the `allow_resume` and
   `allow_response_redaction` publication-workflow toggles.
...
10. **Docs updates:** this file, `docs/groups-view.md` (Organise page
    ...
    `docs/privacy-notice.md` and `docs/compliance/data-subject-request-procedure.md`
    (opt-out token for public surveys — board-approved DSPT material,
    update alongside implementation).

## Publication workflow toggle

A per-survey toggle in the publication workflow controls whether
participants can resume their progress and whether public-survey
participants are offered an opt-out token after submission. This sits
alongside the existing visibility and audience declarations.

```python
class Survey(models.Model):
    ...
    # [Planned] Default True. Creators can disable for surveys with a
    # fresh-state requirement (participant must answer from scratch each
    # time) or where offering a redaction token is inappropriate.
    allow_resume = models.BooleanField(default=True)

    # [Planned] Default True. Controls whether public-survey participants
    # are offered an opt-out (receipt) token on the thank-you page.
    # Authenticated and token surveys already issue a receipt_token
    # where pseudonymous, regardless of this toggle.
