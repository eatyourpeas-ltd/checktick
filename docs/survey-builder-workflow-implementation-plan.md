...
- **First-run nudge — focus the Add Question form** ✅ (originally Tier 3.3 in the old plan). When the builder loads and the active section has no questions, the "Add Question" text input is auto-focused so new users can start typing immediately without reading instructions. Implementation: `builder_question_pane.html` adds `data-autofocus` to the question text `<input>` when `{% if not questions %}`; `builder.js` `focusFirstQuestionInput()` focuses it on `DOMContentLoaded` and after HTMX section switches (`#builder-main` swap re-renders the form). It also fires after a `#questions-list` swap that leaves the list with no `[data-qid]` rows (e.g. deleting the last question), so the input re-focuses when the user empties a section mid-session. Tests: `test_builder_first_run_nudge_marks_input_autofocus_when_empty`, `test_builder_first_run_nudge_no_autofocus_when_section_has_questions`, `test_builder_first_run_nudge_autofocus_on_section_switch_to_empty`.
