...
- **Manual tier upgrades:** Platform admins can manually upgrade a free account to a paid tier with a configurable renewal date via `views_platform_admin.organization_create` (when `create_target == 'account'`). The renewal date is stored on `UserProfile.subscription_current_period_end`, so the existing `process_expired_subscriptions` cron auto-downgrades the account when the date passes. Pre-expiry warnings are sent by the daily `process_expiring_subscriptions` command at 1 month/week/day before expiry (idempotent via `UserProfile.last_expiry_warning_stage`). Re-subscription re-opens surveys auto-closed by a previous downgrade (flagged via `Survey.closed_by_downgrade`) via `UserProfile.reopen_surveys_on_upgrade`. Lapsed users see a `messages.warning()` flash on login (within 14 days) and a LAPSED badge in `base.html`/`profile.html`.
