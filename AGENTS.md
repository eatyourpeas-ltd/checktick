...
- **Annual billing:** `settings.ANNUAL_DISCOUNT_PERCENT` (env: `ANNUAL_DISCOUNT_PERCENT`, default 20) applies a discount to monthly × 12 for annual billing. Each fixed-price tier has a `billing_cycles` sub-dict with `monthly` and `annual` entries. The pricing helpers (`get_tier_amounts`, `get_effective_tiers`, `get_effective_tier_amounts`) accept a `billing_cycle` parameter. VAT is computed on the discounted annual ex-VAT amount.
...
- **Organisation annual billing:** `Organization.billing_cycle` (monthly/annual) is set by the platform admin per-org. `compute_annual_amount()` in `pricing.py` applies the discount to bespoke per-seat/flat-rate amounts.
- **Billing cycle switching:** `views_billing.switch_billing_cycle` cancels the old GoCardless subscription and creates a new one with the new cycle, reusing the existing mandate. GoCardless doesn't support changing `interval_unit` on an existing subscription.
- **Pro-rata refunds:** `compute_pro_rata_refund()` in `pricing.py` calculates the refund for annual subscriptions as `amount × (days_remaining / total_days)`. Monthly subscriptions always refund the full amount. The platform admin refund view allows partial refunds for annual only.
- **Manual tier upgrades:** Platform admins can manually upgrade a free account to a paid tier with a configurable renewal date via `views_platform_admin.organization_create` (when `create_target == 'account'`). The renewal date is stored on `UserProfile.subscription_current_period_end`, so the existing `process_expired_subscriptions` cron auto-downgrades the account when the date passes. Pre-expiry warnings are sent by the daily `process_expiring_subscriptions` command at 1 month/week/day before expiry (idempotent via `UserProfile.last_expiry_warning_stage`). Re-subscription re-opens surveys auto-closed by a previous downgrade (flagged via `Survey.closed_by_downgrade`) via `UserProfile.reopen_surveys_on_upgrade`. Lapsed users see a `messages.warning()` flash on login (within 14 days) and a LAPSED badge in `base.html`/`profile.html`. Manually upgraded accounts get a 7-day grace period before downgrade (matching the GoCardless past-due grace period). Manual tier changes are logged via `AuditLog` and a notification email is sent via `send_manual_upgrade_email`.
- **Layout gating:** Free tier users can only use the linear survey layout. All paid tiers (Pro, Team, Organisation, Enterprise) get every layout: section menu, RCT, guided, staged, matrix, and Delphi. Gated via `TierLimits.allowed_layouts` in `tier_limits.py` and enforced in the organise page view (`survey_groups`), the bulk upload import, and the organise page template (disabled cards show an "Upgrade to use" link). The `check_layout_permission()` and `get_allowed_layouts()` helpers are the single source of truth for layout access checks.
- **Docs:** `docs/billing-and-subscriptions.md` and `docs/pricing-summary.md` describe the default prices (assuming `VAT_RATE=0.20`, `BASE_SEAT_PRICE_EX_VAT=20`, and `ANNUAL_DISCOUNT_PERCENT=20`). Update both whenever the defaults change.
- **Env files:** `.env.example` and `.env.selfhost` document `VAT_RATE`, `BASE_SEAT_PRICE_EX_VAT`, `ANNUAL_DISCOUNT_PERCENT`, `VAT_NUMBER`, `COMPANY_NAME`, `COMPANY_ADDRESS`, `COMPANY_REGISTRATION_NUMBER`.
