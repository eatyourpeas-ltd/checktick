...
# ===========================================================================
# S15 — CSV formula injection (spreadsheet injection)
#
# Answers beginning with =, @, +, or - are interpreted as formulas by
# Microsoft Excel and LibreOffice Calc when opened from a CSV file.  A
# malicious respondent could submit =CMD|'calc'!Z0 or =HYPERLINK(...) to
# execute code on the survey owner's machine when they open the export.
#
# The fix must prefix any such value with a single-quote (') so spreadsheet
# applications treat it as a literal string.
# ===========================================================================


class TestCSVFormulaInjectionPrevention:
    """
    S15 — verify that _format_answer_for_export strips formula-injection
    prefixes from respondent-supplied free-text answers.
    """
    ...
    def test_safe_answer_unchanged(self):
        ...
        result = _format_answer_for_export("John Smith", "text")
        assert result == "John Smith"

    def test_safe_numeric_string_unchanged(self):
        """A numeric string that doesn't start with a formula char is safe."""
        result = _format_answer_for_export("42", "text")
        assert result == "42"
