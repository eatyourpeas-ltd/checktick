...
class TestOrgAdminDashboardAccess:
    ...
    def test_regular_user_without_context_redirected(self, client, regular_user, disable_rate_limiting):
    ...
    def test_org_owner_can_access_dashboard(
        self, client, org_owner, organization, org_recovery_request, disable_rate_limiting
    ):
    ...
    def test_org_admin_can_access_dashboard(
        self, client, org_admin, organization, org_admin_membership, org_recovery_request, disable_rate_limiting
    ):
...
class TestTeamOwnerDashboardAccess:
    ...
    def test_team_owner_can_access_dashboard(
        self, client, team_owner, standalone_team, team_recovery_request, disable_rate_limiting
    ):
        """Team owner can access their team's recovery dashboard."""
        client.force_login(team_owner)
        url = reverse("surveys:admin_recovery_dashboard") + f"?team={standalone_team.id}"
        response = client.get(url)
        assert response.status_code == 200
...
class TestScopedRequests:
    ...
    def test_org_dashboard_only_shows_org_requests(
        self, client, org_owner, organization, org_recovery_request, team_recovery_request, disable_rate_limiting
    ):
    ...
    def test_team_dashboard_only_shows_team_requests(
        self, client, team_owner, standalone_team, org_recovery_request, team_recovery_request, disable_rate_limiting
    ):
...
class TestOrgAdminApproval:
    ...
    def test_org_owner_can_approve_primary(
        ...
        client.force_login(org_owner)
        url = (
            reverse("surveys:admin_recovery_approve_primary", kwargs={"request_id": org_recovery_request.id})
    ...
    def test_dual_approval_requires_different_admins(
        self, client, org_owner, org_admin, organization, org_admin_membership, org_recovery_request, disable_rate_limiting
    ):
        ...
        url = (
            reverse("surveys:admin_recovery_approve_secondary", kwargs={"request_id": org_recovery_request.id})
            + f"?org={organization.id}"
        )
        response = client.post(url)
        org_recovery_request.refresh_from_db()
...
class TestTeamOwnerApproval:
    ...
    def test_team_owner_can_approve_primary(
        ...
        client.force_login(team_owner)
        url = (
            reverse("surveys:admin_recovery_approve_primary", kwargs={"request_id": team_recovery_request.id})
...
class TestRejectAction:
    ...
    def test_org_admin_can_reject(
        ...
        client.force_login(org_owner)
        url = (
            reverse("surveys:admin_recovery_reject", kwargs={"request_id": org_recovery_request.id})
            + f"?org={organization.id}"
        )
...
class TestCrossOrgAccess:
    ...
    def test_org_admin_cannot_approve_team_request(
        self, client, org_owner, organization, team_recovery_request, disable_rate_limiting
    ):
    ...
    def test_team_owner_cannot_approve_org_request(
        self, client, team_owner, standalone_team, org_recovery_request, disable_rate_limiting
    ):
...
class TestDetailView:
    ...
    def test_org_admin_can_view_org_request_detail(
        ...
        client.force_login(org_owner)
        url = (
            reverse("surveys:admin_recovery_detail", kwargs={"request_id": org_recovery_request.id})
            + f"?org={organization.id}"
        )
        response = client.get(url)
        assert response.status_code == 200
    ...
    def test_org_admin_cannot_view_team_request_detail(
        ...
        client.force_login(org_owner)
        url = (
            reverse("surveys:admin_recovery_detail", kwargs={"request_id": team_recovery_request.id})
            + f"?org={organization.id}"
        )
        response = client.get(url)
